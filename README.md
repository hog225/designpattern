...
1. Singleton (싱글톤): 하나의 클래스 인스턴스만을 갖도록 보장하고, 이에 접근할 수 있는 전역적인 지점을 제공합니다.
2. Factory Method (팩토리 메서드): 객체의 생성을 캡슐화하여 서브 클래스에서 인스턴스를 만들 수 있도록 합니다.
3. Abstract Factory (추상 팩토리): 관련된 객체들을 생성하기 위한 인터페이스를 제공하고, 이를 이용하여 여러 팩토리 클래스를 생성할 수 있습니다.
4. Builder (빌더): 복잡한 객체의 생성 프로세스를 분리하여 객체를 생성하는 패턴입니다.
5. Prototype (프로토타입): 기존 객체를 복사하여 새로운 객체를 생성하는 패턴입니다.

### 구조 패턴(Structural Patterns):
1. Adapter (어댑터): 인터페이스가 다른 두 클래스를 함께 동작할 수 있도록 하는 패턴입니다.
2. Bridge (브리지): 구현과 추상을 분리하여 각각 독립적으로 변형할 수 있게 합니다.
3. Composite (컴포지트): 객체들을 트리 구조로 구성하여 개별 객체와 복합 객체를 동일하게 다룰 수 있게 합니다.
4. Decorator (데코레이터): 객체에 추가적인 기능을 동적으로 부여할 수 있도록 하는 패턴입니다.
5. Facade (퍼사드): 서브시스템의 인터페이스를 단순화하여 외부에 제공하는 패턴입니다.
6. Flyweight (플라이웨이트): 객체의 인스턴스를 공유하여 메모리 사용을 최적화하는 패턴입니다.
7. Proxy (프록시): 다른 객체에 대한 접근을 제어하기 위한 대리자나 대변인 역할을 하는 패턴입니다.

### 행동 패턴(Behavioral Patterns):
1. Chain of Responsibility (책임 연쇄): 여러 객체가 처리 요청을 받아서 그 중 하나가 처리할 수 있도록 전달하는 패턴입니다.
2. Command (커맨드): 요청을 객체의 형태로 캡슐화하여 클라이언트가 서로 다른 요청을 매개 변수화하고, 큐에 저장하거나 로그로 기록할 수 있게 합니다.
3. Interpreter (인터프리터): 언어 인터프리터를 만드는 패턴으로, 주어진 언어의 문법을 정의하고 이를 해석하는 역할을 수행합니다.
4. Iterator (이터레이터): 컬렉션의 요소에 순차적으로 접근할 수 있게 하는 패턴입니다.
5. Mediator (미디에이터): 객체들 사이의 상호작용을 캡슐화하여 객체 간의 직접적인 통신을 방지하는 패턴입니다.
6. Memento (메멘토): 객체의 내부 상태를 캡슐화하여 나중에 객체를 그 상태로 복원할 수 있도록 하는 패턴입니다.
7. Observer (옵서버): 한 객체의 상태가 변경될 때 그 객체에 의존하는 다른 객체들에게 알림을 보내는 패턴입니다.
8. State (스테이트): 객체의 내부 상태가 변경될 때 객체의 행동을 변경시키는 패턴입니다.
9. Strategy (스트래티지): 알고리즘군을 정의하고, 각각을 캡슐화하여 교체 가능하게 만드는 패턴입니다.
10. Template Method (템플릿 메서드): 알고리즘의 구조를 메서드에 정의하고, 하위 클래스에서 알고리즘의 구조를 변경할 수 있게 하는 패턴입니다.
11. Visitor (비지터): 객체 구조를 정의하고, 이 구조를 탐색하는 연산을 구분하는 패턴입니다.
